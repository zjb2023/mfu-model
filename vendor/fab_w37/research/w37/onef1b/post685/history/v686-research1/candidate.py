...
def run_candidate(out,aligned,pairs,phase,gpu,wrapper,paths,guard):
    ...
    specs=[('v686_split_full',[85,90],'full',True),('v686_split_zero_runtime',[85,90],'zero_runtime',True),
           ('v686_split_sender_only',[85,90],'sender_only',True),('v686_split_ordinal',[85,90],'full',False),
           ('v686_full4',[85,90,95,100],'full',True)]
    for name,fit,kind,role in specs:
        for pp,mb,case in [(16,4,'source256'),(14,3,'target224')]:
            cost=FittedCosts(aligned,pairs,fit,pp,mb,role_transfer=role)
