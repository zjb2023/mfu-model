...
def run_candidate(out,aligned,pairs,phase,gpu,wrapper,paths,guard,study='v688'):
    ...
    if study=='v686':
        specs=[('v686_split_full',[85,90],'full',True,FittedCosts),('v686_split_zero_runtime',[85,90],'zero_runtime',True,FittedCosts),
               ('v686_split_sender_only',[85,90],'sender_only',True,FittedCosts),('v686_split_ordinal',[85,90],'full',False,FittedCosts),
               ('v686_full4',WINDOW,'full',True,FittedCosts)]
    elif study=='v688':
        specs=[('v687_split_full',[85,90],'full',True,ReadinessCosts)]+[(f'v688_member{it}',[it],'full',True,ReadinessCosts) for it in WINDOW]
    ...
    if study=='v688':
        from scenarios import aggregate
        sm=aggregate(out,records,phasepred,sourcegraphs)
    ...
    if study=='v688':
        from diagnostics import attribution
        attribution(out,targetgraphs['v687_split_full'],partitions,paths)
    for item in seal['files']:assert sha(out/item['path'])==item['sha256']
