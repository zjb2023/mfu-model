...
def evaluate(out,records,paths,partitions):
    ...
    members={}
    if (out/'prediction/scenario_spans.csv').exists():
        spans=pd.read_csv(out/'prediction/scenario_spans.csv')
        members={r.variant:r.members.split(',') for r in spans[spans.case.eq('target224')].itertuples()}
    grouped=stage_envelopes(pred,members)
