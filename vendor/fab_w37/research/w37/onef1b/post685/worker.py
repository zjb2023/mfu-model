...
def csv(out,name,d):
    ...
    d.to_csv(p,index=False,compression=compression)
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--mode',required=True);p.add_argument('--study',default='v688');p.add_argument('--output',type=Path,required=True);a=p.parse_args();out=a.output.resolve()
    ...
    if a.mode=='candidate':
        names2=['dag_v685_nodes.csv.gz','dag_v685_edges.csv.gz','iteration_ground_truth.csv','dag_v682_stage_aware_pp_gradient_payload.json','target224_outside_wrapper_ground_truth.csv.gz','target_phase_rank_events_60_100.csv']
    ...
    summary={'status':'PASS_SOURCE_STATIC_API_ORDER_AND_DISJOINT_PARTITIONS','source_actions':len(aligned),'source_api_rows':len(api),
        ...
        'first_round_formal_topology_unchanged':True,'target_timing_read_during_source_audit':False}
    dump(out/'audit/summary.json',summary)
    if a.mode=='candidate':
        from candidate import run_candidate
        summary['candidate']=run_candidate(out,aligned,pairs,phase,gpu,wrapper,paths,guard,study=a.study)
        summary['target_timing_read_after_prediction_seal']=True
