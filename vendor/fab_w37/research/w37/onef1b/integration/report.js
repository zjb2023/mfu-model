...
const scope=document.getElementById('result-scope');function updateScope(){document.querySelectorAll('[data-result-scope]').forEach(el=>el.hidden=el.dataset.resultScope!==scope.value)}scope.addEventListener('change',updateScope);updateScope();
...
document.getElementById('download-html').addEventListener('click',()=>{const clone=document.documentElement.cloneNode(true);const blob=new Blob(['<!doctype html>\n'+clone.outerHTML],{type:'text/html;charset=utf-8'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='W37_分布式训练与MFU性能预测.html';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),5000)});
