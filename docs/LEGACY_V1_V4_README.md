# 历史说明：256 卡 v1–v4 Trace-driven MFU model

以下保留旧版 README。文中的“当前版本”、服务状态及外部路径均属于历史记录，
不代表 `feat/w37-v610` 当前状态。命令路径以仓库根目录为基准；不要将旧 OISA
运行链用于 v610 冻结复现。当前入口见[仓库首页](../README.md)。

中文入门说明：[三阶段 MFU 模型与基于计算图的 MFU 模型说明](MFU_THREE_STAGE_AND_DAG_MODEL_GUIDE.md)。该文档面向首次接触本工程的读者，解释三阶段视角、计算图建模、OISA 网络仿真边界和历史精度口径。
...
See [v6.10 reproduction and acceptance](v610/README.md).
